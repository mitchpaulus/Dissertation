...
With historical data, for each timestep the terminal unit flows and the
damper positions in the box is known. There are 3 equations with 6
...
drop relationships, this method loses the potential usefulness. For this
...
In order to predict the energy use at the cooling/reheat coils in the
...
Since the system is not going to expected to determine semi-optimal
...
hundreds.  This is extremely feasible for modern computers to handle in
the sub-second time range. 
...
capacity of the cooling coils.  This may be on the order of
\SI{50}{\degreeF}.

...
and location on a schematic. A sample schematic from the NCTM project is
shown in \figref{} \ref{fig:AHUSchematic}. The different types of
equipment such as fans, coils, and temperature sensors are placed on the
schematic and are associated with an actual sensor. For example, a
temperature sensor that is at the outlet of the air handling unit,
unaffected by any other temperature affecting device, is defined to have
the \textit{supply air temperature} label. 
...
Each component is related to a \textit{container}, in which trends can
be associated with.  A building container is a parent to air handling
unit containers, and air handling unit containers are parents to
terminal unit containers.  \figref{} \ref{fig:ContainerHierarchy} shows
a portion of the hierarchy for the NCTM project. Each line in \figref{}
\ref{fig:ContainerHierarchy} is a different container. The indentation
indicates the relationships. By organizing trends
into containers which have parent-child relationships, along with
